...
class PixelWiseA3C_ConvGRU:
        ...
        with torch.no_grad():
            ...
            while isEnd == False:
                ...
                for _ in range(0, self.t_max):
                    ...
                    noise_image = torch.as_tensor(current_state.tensor).to(self.device)
                    pi, _, inner_state = self.model(noise_image)
    ...
    def train(self, train_set, test_set, batch_size, episodes, save_every):
        ...
        while cur_episode < max_episode:
            ...
            sum_reward, loss = self.train_step(noise_image, labels)
    ...
    def train_step(self, noise, labels):
        ...
        for t in range(0, self.t_max):
            ...
            pi, v, inner_state = self.model(noise)
