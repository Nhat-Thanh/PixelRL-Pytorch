...
class State:
    ...
    def step(self, act, inner_state):
        ...
        b = action.shape[0]
        for i in range(0, b):
            if np.sum(action[i] == 3):
                gaussian[i, 0] = cv2.GaussianBlur(self.image[i, 0],  ksize=(5,5), sigmaX=0.5)

            if np.sum(action[i] == 4):
                bilateral[i, 0] = cv2.bilateralFilter(self.image[i, 0], d=5, sigmaColor=0.1, sigmaSpace=5)

            if np.sum(action[i] == 5):
                median[i, 0] = cv2.medianBlur(self.image[i, 0], ksize=5)

            if np.sum(action[i] == 6):
                gaussian_2[i, 0] = cv2.GaussianBlur(self.image[i, 0], ksize=(5,5), sigmaX=1.5)

            if np.sum(action[i] == 7):
                bilateral_2[i, 0] = cv2.bilateralFilter(self.image[i, 0], d=5, sigmaColor=1.0, sigmaSpace=5)

            if np.sum(action[i] == 8):
                box[i, 0] = cv2.boxFilter(self.image[i, 0], ddepth=-1, ksize=(5,5))

        self.image = moved_image 
        self.image = np.where(action[:,np.newaxis,:,:]==3, gaussian,    self.image)
        self.image = np.where(action[:,np.newaxis,:,:]==4, bilateral,   self.image)
        self.image = np.where(action[:,np.newaxis,:,:]==5, median,      self.image)
        self.image = np.where(action[:,np.newaxis,:,:]==6, gaussian_2,  self.image)
        self.image = np.where(action[:,np.newaxis,:,:]==7, bilateral_2, self.image)
        self.image = np.where(action[:,np.newaxis,:,:]==8, box,         self.image)
